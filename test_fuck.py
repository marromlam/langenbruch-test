...
def mass_model(mass, prob, fsig=1, fexp=0,
               a=0, b=0, c=0,
               norm=1, xLL=None, xUL=None, *args, **kwargs):
...
if __name__ == '__main__':
  ...
  pars.add({"name": "a", "value": 2,
            "free": False,
            "latex": r"\mu"})
  pars.add({"name": "b", "value": 0.3, #  "min":0, "max":1,
            "free": True,
            "latex": r"\sigma"})
  pars.add({"name": "c", "value": np.pi/2, "free": True,
            "latex": "a_l"})
  pars.add({"name": "d", "formula": "2*c",
            "latex": "a_l"})
  ...
  if not os.path.exists("test_fuck.root"):
    ...
    with uproot.recreate("test_fuck.root") as f:
      _branches = {}
  ...
  sample = ipanema.Sample.from_root("test_fuck.root")
